...
Woodcutting / stonecutting / recycling. Preço "vanilla" preservado (calibragem pré-calculada do ref). ~409 receitas no ref; na 26.2 reimplementamos só o que **não** é vanilla. Pacote **completo**.
...
- ✅ **Copper cutting** (stonecutter) — 40 receitas: bloco de cobre do estado → `copper_bars` (24), `copper_chain` (8), `lightning_rod` (3), `copper_door` (5) e `copper_trapdoor` (4), nos 8 estados de oxidação. São as cinco famílias que a vanilla 26.2 **não** corta; `cut_copper`/stairs/slab/chiseled/grate ficam com a vanilla. Nenhum desses produtos é reciclável no RootBoot nem na vanilla (os únicos consumidores vanilla aplicam cera), então nada disso realimenta material.
- ⚠️ **"Preço vanilla" do cobre não é custo exato:** medidos contra a bancada, os rendimentos do ref divergem — barras (24 = 9 lingotes) e haste (3 = 9 lingotes) empatam, corrente (8 = 9,78) e porta (5 = 10) saem levemente baratas, e o **alçapão (4 = 16 lingotes) sai 78% mais barato**. É a calibragem do ref, mantida verbatim por decisão da issue #32; o desconto é intencional e não forma ciclo.
...
- ✅ #12 **Drop Ladder** `(S)` — preserva o comportamento da referência: enquanto está numa coluna de escadas e segura uma escada em qualquer mão, cada novo pressionamento de agachar coloca exatamente uma unidade abaixo da extremidade inferior. Consome uma escada fora do criativo; se houver escadas nas duas mãos, consome primeiro da mão principal e usa a secundária somente como fallback. Espaço obstruído não coloca nem consome. Escadas penduradas dependem da cadeia acima para permanecer sustentadas. O suporte é a própria regra de sobrevivência da ladder: ela passa a aceitar uma ladder de mesmo facing logo acima, e uma atualização vinda de cima volta a consultá-la — é o que faz a cadeia cair inteira quando a de cima se vai, como já acontece quando a parede se vai. Essa regra fica instalada independentemente do toggle, porque desabilitar a feature deve impedir novas colocações sem derrubar as escadas penduradas que o mundo já tem; como efeito, também passa a ser possível encaixar uma ladder à mão sob outra sem parede, que é como a própria referência descreve a feature.
- ✅ #13 **Totem Works In Void** `(S)` — ao receber dano `out_of_world` em qualquer dimensão vanilla ou modded com um totem em qualquer mão, ativa e consome um totem pelo fluxo vanilla. Por até 60 segundos, o jogador sobe normalmente e desce lentamente enquanto segura agachar. O resgate termina imediatamente ao pousar ou quando o tempo expira; o término encerra apenas o resgate atual, e um novo evento letal de dano `out_of_world` pode consumir outro totem e iniciar um novo prazo completo. O único ponto que mantém o totem fora do vazio é a guarda de `checkTotemDeathProtection` contra a tag `bypasses_invulnerability`; o RootBoot responde essa guarda apenas para `out_of_world`, de modo que a busca nas duas mãos, o consumo, a estatística, o avanço, a vida restaurada, os efeitos e a animação continuam sendo os da vanilla. Enquanto um resgate corre, o dano `out_of_world` daquele jogador é suprimido, e só ele: sem isso o prazo não existiria, porque o vazio fere 4 pontos a cada 10 ticks e o totem devolve o jogador com 1 de vida. Subir e descer são expressos como Levitation VIII e Slow Falling, os efeitos vanilla que já significam isso, de forma que o movimento é previsto pelo cliente que o executa e um cliente sem RootBoot vê exatamente o que acontece. A velocidade da subida — cerca de 7 blocos por segundo — é ditada pela geometria, e não pelo gosto: o totem só dispara no golpe letal, e o vazio leva cinco golpes de 4 pontos espalhados por 40 ticks para chegar nele, então o jogador cai cerca de 157 blocos em velocidade terminal antes de o resgate começar, uns 220 blocos abaixo da altura em que o vazio o alcançou pela primeira vez. Voltar às ilhas do End dentro do minuto, com tempo para agachar e pousar, é o que fixa esse número. O resgate só mexe no que ele mesmo aplicou: subir não limpa Slow Falling, porque Levitation já substitui a gravidade por inteiro, e um efeito que o jogador já tinha de outra origem é deixado intacto em vez de sobrescrito. **Limite conhecido:** no Overworld o piso do dano fica abaixo da bedrock, então um resgate iniciado lá sobe até encostar nela; no End, de onde o vazio é efetivamente alcançável, a subida é livre.
